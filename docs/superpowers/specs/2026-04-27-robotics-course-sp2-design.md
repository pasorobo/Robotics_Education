...
| 想定総ファイル数 | **設計完了時 26 新規 + 3 修正 = 29、writing-plans 後 27 新規 + 3 修正 = 30** |
| 想定総行数 | 約 2,500 行 (新規分) |
...
| Sandbox reference week2/ (11 ファイル) | 各 Lab の real run 軽量証跡 + 2 template 記入例 + codex_prompt_lab4b.md |
...
- `course/week2/` 完成 (Lecture 2 + Lab 3 + Template 2 + README + sandbox_reference 11 件)
...
**設計完了時点: 新規 26 / 修正 3 / 計 29 ファイル**。`writing-plans` skill 実行後に **plan ファイル 1 件追加** され、最終 SP2 着手時は新規 27 + 修正 3 = 30 ファイル。`docs/references.md` の修正は条件付きで増加可能性あり (§2.3 #28 参照)。
...
#### E. Sandbox Reference Week 2 (11 ファイル)
...
| C. Templates | 2 | 0 |
| D. Week README | 1 | 0 |
| E. Sandbox reference | 11 | 0 |
| F. Tools | 0 | 2 |
| G. Root/docs | 0 | 1 |
| H. Spec | 1 | 0 |
| **合計 (design 完了時点)** | **26 新規** | **3 修正** |
...
### 3.8 Sandbox reference week2/ 内容仕様 (新規 11 ファイル)
...
instructor walk-through 完了 = `sandbox_reference/week2/` 配下の **全 11 ファイル** が commit され、`bash tools/check_structure.sh` が PASS の状態。
...
|---|---|---|---|
| **G1: 構造ゲート** | SP1 + SP2 expected files (**43 + 26 = 69 ファイル** + plan 1 = 70) すべて存在 | `bash tools/check_structure.sh` | EXPECTED_FILES に W2 26 件追加、SP1 fix I-5 由来の `sandbox_reference/week1/lab1/README.md` を expected list に正規化 |
...
# 2. tools/verify_env.sh の --week 2 モードを実装 (SP2 design Task 0)
bash tools/verify_env.sh --week 2

# 3. tools/check_structure.sh を SP2 expected files に拡張 (SP2 design Task 0)
bash tools/check_structure.sh

# 4. SP1 互換性回帰確認 (--week 引数なしで SP1 同等の挙動)
bash tools/verify_env.sh   # SP1 互換、Gazebo 必須・FAIL の可能性あり (SP1 既知)
```
...
5. **コスト**: 26 新規 + 3 修正 = 29 ファイルが Q1 進行 (W2 = 2026-05-04) に間に合うか
...
- SP2 完了時 expected files: **70 件 (SP1 ベース 43 + SP2 新規 26 + plan 1)**
- Q1 reduced Lv1 W2 開始日 (2026-05-04) に間に合う
...
| F5 (sec4) | codex_prompt_lab4b.md 必須化 | sandbox_reference に必須ファイル追加、E. 11 件、合計 26 新規 | §2.2 #24, §2.4 |
...
| F4 (sec5) | G1 base 43 + 26 = 69 統一 | SP1 fix I-5 由来 `sandbox_reference/week1/lab1/README.md` を expected list に追加 | §2.3 #28, §5.1 |
