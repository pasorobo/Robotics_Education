...
> **7-key vs 10-key の使い分け (read once)**: 7-key は `docs/superpowers/specs/` および `docs/superpowers/plans/` 配下の SP5 spec / plan 自体の front matter 規約。10-key は `course/role_tracks/` 配下の index README + 7 stub README (course/reference 系 stub 文書) の front matter 要件。両者は併存しており、適用先が異なる。
...
> **Audit 用一行式 (count drift 防止)**: `56 + 7 + 6 + 4 + 2 + 3 = 78`。将来 diff で内訳が変わった場合、本式と上表を同時更新すること。
